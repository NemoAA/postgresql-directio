...
#ifdef DIRECT_IO
	/*TODO: woonhak check buffer status */
   posix_memalign((void**) &page, ALIGNOF_SECTOR, BLCKSZ);
#else
...
_bt_blwritepage(BTWriteState *wstate, Page page, BlockNumber blkno)
{
	...
	while (blkno > wstate->btws_pages_written)
	{
     ...
     if (!wstate->btws_zeropage){
       posix_memalign((void**) &(wstate->btws_zeropage), ALIGNOF_SECTOR, BLCKSZ);
       memset((void *) wstate->btws_zeropage, 0X00, BLCKSZ);
     }
#else
     if (!wstate->btws_zeropage)
...
_bt_uppershutdown(BTWriteState *wstate, BTPageState *state)
{
	 ...
	 */
#ifdef DIRECT_IO
	posix_memalign((void**) &(metapage), ALIGNOF_SECTOR, BLCKSZ);
#else
