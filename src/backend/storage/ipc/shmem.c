...
#ifdef DIRECT_IO
/*
 * ShmemInitStructBlock -- Create/attach to a structure in shared memory.
 *
 *		This is called during initialization to find or allocate
 *		a data structure in shared memory.  If no other process
 *		has created the structure, this routine allocates space
 ...
 */
void *
ShmemInitStructBlock(const char *name, Size size, bool *foundPtr)
{
	ShmemIndexEnt *result;
	void	   *structPtr;
	...
	if (!ShmemIndex)
	{
		...
		else
		{
			 ...
			 */
			Assert(shmemseghdr->index == NULL);
			structPtr = ShmemAllocBlock(size);
			if (structPtr == NULL)
				ereport(ERROR,
						(errcode(ERRCODE_OUT_OF_MEMORY),
						 errmsg("not enough shared memory for data structure"
...
/*
 * ShmemAllocBlock -- allocate max-aligned chunk from shared memory
 *
 * Assumes ShmemLock and ShmemSegHdr are initialized.
 *
 * Returns: real pointer to memory or NULL if we are out
